    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="shadow overflow-hidden rounded-md bg-white p-4">
                <form action="{{ route('summary.update', $row->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        ...
                        <select name="skill_id" id="skill_id" class="appearance-none border border-gray-200 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('skill_id') border-red-500 @enderror">
                            ...
                            @foreach($skills as $skill)
                                <option value="{{ $skill->id }}" {{ $row->skill_id == $skill->id ? 'selected' : '' }}>{{ $skill->name }}</option>
                            @endforeach
                        </select>
                        @error('skill_id')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" class="appearance-none border border-gray-200 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror">{{ $row->description }}</textarea>
                        @error('name')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
