    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-end mb-2">
                <a href="{{ route('summary.create') }}" class="inline-block bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded mr-2">Add New</a>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-100 w-full">
                    <thead class="bg-gray-50 shadow-thead">
                        <tr>
                            ...
                            <th class="p-2 text-left text-sm font-medium text-color-2">Skill</th>
                            <th class="p-2 text-left text-sm font-medium text-color-2">Description</th>
                    ...
                    <tbody class="bg-white">
                        @foreach($data as $row)
                            <tr class="border-t border-b border-gray-100">
                                ...
                                <td class="p-2 text-color-2">{{ $row->skill->name }}</td>
                                <td class="p-2 text-color-2">{!! $row->description !!}</td>
                                <td class="p-2 flex items-center justify-center">
                                    <a href="{{ route('summary.edit', $row->id) }}" class="inline-block bg-yellow-500 hover:bg-yellow-600 p-1 text-white rounded mr-2">
                                    ...
                                    <form action="{{ route('summary.destroy', $row->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
